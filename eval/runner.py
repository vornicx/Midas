...
def _load(
    ...
    *,
    ...
    if dataset == "synthetic-es":
        return synthetic_es()
    if dataset == "beam":
        return beam(path=beam_path, tier=beam_tier, max_conversations=max_convs or None, merge_rounds=beam_merge_rounds, dual_rounds=beam_dual_rounds)
    if dataset == "multiday":
        return multiday()
...
def main() -> None:
    ...
    parser.add_argument("--beam-dual-rounds", type=int, default=0, help="BEAM: dual-granularity — index single turns AND N-turn rounds together (0 = off)")
    ...
    dataset = _load(
        ...
        beam_dual_rounds=args.beam_dual_rounds,
    )
    ...
    midas = MidasAdapter(
        ...
        anchor_boost=args.midas_anchor_boost,
