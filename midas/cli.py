...
def _cli_add(cmd: list[str], *, dry: bool) -> str | None:
    """Configure a client via its own CLI (claude/codex). None if that CLI isn't installed."""
    ...
    try:
        ...
        return f"configured via `{cmd[0]} mcp add`"
    except subprocess.CalledProcessError as e:
        tail = (e.stderr or e.stdout or "").strip().splitlines()
        return f"`{cmd[0]}` failed: {tail[-1][:90] if tail else 'error'}"
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    r = _cli_add(["claude", "mcp", "add", "midas", "-s", "user",
                  "-e", "MIDAS_MCP_EMBEDDER=local", "--", "midas-mcp"], dry=args.dry_run)
    if r:
        results.append(("Claude Code", r))
    r = _cli_add(["codex", "mcp", "add", "midas", "--", "midas-mcp"], dry=args.dry_run)
    if r:
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    print("clients:")
    checks = [("Claude Code", Path.home() / ".claude.json"),
              ("Cursor", Path.home() / ".cursor/mcp.json"),
              ("Codex", Path.home() / ".codex/config.toml"),
              ("Windsurf", Path.home() / ".codeium/windsurf/mcp_config.json")]
    cd = _claude_desktop_path()
    if cd:
        checks.append(("Claude Desktop", cd))
